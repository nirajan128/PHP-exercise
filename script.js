...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateResult(formId, resultId, phpFile) {
    const theForm = document.getElementById(formId);
    theForm.addEventListener("submit", (event) => {
      event.preventDefault();
  ...
  generateResult("diceRollForm", "result10", "dice_roll.php");
  generateResult("fancierDiceRollForm", "result11", "fancier_dice.php");
  generateResult("passwordForm", "result13", "generate_pass.php");

  //Day and night
  fetch("day_or_night.php")
    .then((response) => response.text())
    .then((data) => {
      // Update the result on the page
